...
def _safe_float(val: Any) -> float:
    """Coerce finite numeric input to float, returning 0.0 otherwise."""
    try:
...
def _safe_int(val: Any) -> int:
    """Coerce finite numeric input to int, returning 0 otherwise."""
    try:
...
class InsightsEngine:
    ...
    def _compute_skill_breakdown(self, skill_usage: List[Dict]) -> Dict[str, Any]:
        ...
        top_skills.sort(key=lambda s: (s["total_count"], s["view_count"], s["manage_count"], _safe_float(s["last_used_at"]), s["skill"]), reverse=True)
        return {
