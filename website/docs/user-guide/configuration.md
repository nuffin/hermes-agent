...
display:
  ...
  interim_assistant_messages: true  # CLI, gateway, and Desktop: show natural mid-turn assistant updates
...
`interim_assistant_messages` controls whether Hermes surfaces completed mid-turn assistant updates. The classic CLI renders them in scrollback between tool calls, gateway platforms send them as separate chat messages, and Desktop keeps them in the transcript instead of collapsing the bubble to only the final message. This is independent from `tool_progress` and does not require streaming.
