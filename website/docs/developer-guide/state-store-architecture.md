...
The currently implemented session/message slice has a transactionally locked, catalog-validated migration ledger: v1 creates sessions/messages, v2 adds session metadata and indexes, v3 records the parent-session foreign key, v4 is a documented compatibility checkpoint validating the previously unledgered v1–v3 contract, v5 adds title fields/indexes, v6 adds visibility fields/indexes, and v7 adds the bounded active-message record columns. Every recorded version is revalidated against PostgreSQL catalogs on open; missing or mismatched objects fail closed rather than being silently trusted. The v4 checkpoint intentionally has no DDL because it only makes the legacy parent-key transition durably auditable. The v7 record contract preserves full appendable message fields and atomic batches, but does not claim SessionDB rewrite, compaction, lineage/resume projection, FTS, or runtime-consumer parity. This remains a bounded slice, not a full state.db upgrade or SQLite-to-PostgreSQL import contract.
