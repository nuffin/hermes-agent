...
`doctor()` is sanitized: it returns no DSN or credentials. It proves loopback reachability, PostgreSQL server version, required extension availability, and the exact current Alembic head `state_store_v27_session_topics`. The exact contract is: **v25 immutable historic core baseline -> v26 non-topic sqlite import manifest -> v27 topic-owned catalog** (`state_store_v25_core` → `state_store_v26_sqlite_import` → `state_store_v27_session_topics`). Its read-only semantic validation covers the v1–v25 core catalog, `sqlite_import_manifests`, and the v27-owned `session_topics`/`messages.topic_id` contract: exact tenant relations, columns/types/nullability/defaults/generated expression, PK/UQ/FK semantics (including same-tenant FK targets) and FK validation state, and non-primary index method/key/predicate/unique/ready/live/valid state. It then reports required-table inventory/counts, message and usage FK-orphan checks, generated-document/GIN search health, active ownership-lease count, and delivery-ledger v1/catalog/count when present. A legacy `schema_migrations` ledger, malformed Alembic metadata, a v25 or v26 tenant that cannot be upgraded to the current head, catalog drift, unhealthy search index, or orphan invariant fails closed; no SQLite fallback is permitted. Historic raw numeric v26 remains distinct from the non-topic Alembic manifest revision.
...
`tests/integration/test_postgresql_state_store_operations.py` proves the loopback named-volume PG18 operations path, and `tests/integration/test_postgresql_state_store_core_alembic.py` covers destructive-fixture-only bootstrap from the historic v25 baseline and authentic v26 non-topic manifest through the current v27 topic-owned head, plus legacy-ledger refusal:
