...
- Real cmd_sessions handler: CLI --sort flag > runtime config > schema default
...
class TestCmdSessionsSort:
    ...
    def test_no_sort_flag_defaults_to_last_active(self):
        """No --sort flag → order_by_last_active=True (schema default)."""
    ...
    def test_schema_default_when_no_cli_no_config(self, monkeypatch):
    ...
    def test_invalid_config_warns_and_falls_back_to_last_active(self, caplog):
        """A bad manual YAML value must not silently select creation-time ordering."""
        ...
        mock_db = self._make_mock_db()
        with caplog.at_level(logging.WARNING, logger="hermes_cli.sessions_cmd"):
            self._run(self._make_args(sort=None), mock_db)
        assert mock_db.list_sessions_rich.call_args.kwargs["order_by_last_active"] is True
        assert "Ignoring invalid sessions.list_sort='newest'" in caplog.text
...
class TestConfigKey:
    ...
    @pytest.mark.parametrize("value", ["newest", "", None, False])
    def test_config_validation_rejects_unsupported_values(self, value):
        from hermes_cli.config import validate_config_structure
        issues = [
            issue for issue in validate_config_structure({"sessions": {"list_sort": value}})
            if "sessions.list_sort" in issue.message
        ]
        assert len(issues) == 1
        assert issues[0].severity == "error"
        assert "'started' or 'last-active'" in issues[0].message
