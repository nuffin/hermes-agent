...
def test_subagent_cap_uses_commit_model():
    """before_call checks cap; a committed reservation charges the counter.

    The counter only increases after successful child construction.
    before_call alone does not charge. (#72550)
    """
    from agent.tool_guardrails import _set_active_subagent_guardrail, reserve_subagent_spawns
    controller = ToolCallGuardrailController(
    ...
    with reserve_subagent_spawns(3) as reservation:
        reservation.commit()
