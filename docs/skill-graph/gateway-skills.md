...
- **驻留于 `$HERMES_HOME/skills/`**：作为 symlink 指向实际 skill 目录，实现 profile 隔离的扁平化索引
...
当 `config.yaml` 中 `agent.skill_graph_mode: true` 且 `skill_graph_search`、`skill_load`
两个工具都成功注册时，plugin 会通过 cache-safe system-prompt section 注入以下内容；否则保留原来的 flat index：
...
`skill-graph` 是默认显式列出的 gateway skill，并作为其他 skill 的入口点。
配置 `extensions_file` 后，其中 `Pre-installed Gateways` 表格声明的额外 gateway skills
也会加入这个最小列表；不会展开整个 flat index。

相关代码：`plugins/skill-graph/__init__.py` → `_render_skill_graph_prompt()`
...
Gateway skills 通过 symlink 机制实现零延迟。将 skill 目录链接到当前 profile 的
`$HERMES_HOME/skills/` 下：

```bash
# 将 intent-router 从 personal-suite 链接到当前 profile
ln -s ~/studio/hermes/projects/hermes-personal-suite/skills/intent-router \
      "$HERMES_HOME/skills/hermes/intent-router"

# 将 post-flight 链接进来
ln -s ~/studio/hermes/projects/hermes-personal-suite/skills/post-flight \
      "$HERMES_HOME/skills/hermes/post-flight"

# 将 quality-gate 链接进来
ln -s ~/studio/hermes/projects/hermes-personal-suite/skills/quality-gate \
      "$HERMES_HOME/skills/software-development/quality-gate"
```
...
| 特性 | Symlink → `$HERMES_HOME/skills/` | `source_dirs` |
|------|------------------------------|---------------|
