...
```bash
ln -s <source-skill-dir> "$HERMES_HOME/skills/<category>/<skill-name>"
```
...
- `agent/system_prompt.py` — System prompt 组装和 cache 边界
- `plugins/skill-graph/__init__.py` — graph prompt section 和工具注册
