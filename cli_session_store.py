...
class PostgreSQLCLISessionStore:
    ...
    def get_meta(self, key: str): return self._store.get_meta(key)
    def set_meta(self, key: str, value: str, **kwargs: Any): return self._store.set_meta(key, value, **kwargs)
    def list_meta_prefix(self, prefix: str): return self._store.list_meta_prefix(prefix)
    def archive_and_compact(self, *args: Any, **kwargs: Any): return self._store.archive_and_compact(*args, **kwargs)
    def session_unread(self, session_row: Mapping[str, Any] | None): return self._store.session_unread(session_row)
    def set_session_read(self, session_id: str, read: bool = True): return self._store.set_session_read(session_id, read)
    def session_yolo_enabled(self, session_meta: Mapping[str, Any] | None): return self._store.session_yolo_enabled(session_meta)
    def set_session_yolo(self, session_id: str, enabled: bool): return self._store.set_session_yolo(session_id, enabled)
    def refresh_auto_title(self, session_id: str, title: str, *, source: str): return self._store.refresh_auto_title(session_id, title, source=source)
    def reopen_if_explicitly_closed(self, session_id: str, *, provenance: str, patience_s: float | None = None): return self._store.reopen_if_explicitly_closed(session_id, provenance=provenance, patience_s=patience_s)
    def is_explicit_fork_child(self, session_id: str): return self._store.is_explicit_fork_child(session_id)
    def get_compression_chain(self, session_id: str): return self._store.get_compression_chain(session_id)
    ...
    def archive_stale_sessions(self, idle_days: float, *, exclude_pinned: bool = True): return self._store.archive_stale_sessions(idle_days, exclude_pinned=exclude_pinned)
    def unarchive_recoverable_session(self, session_id: str): return self._store.unarchive_recoverable_session(session_id)
    def prune_empty_ghost_sessions(self, sessions_dir: Any = None): return self._store.prune_empty_ghost_sessions(sessions_dir)
    def maybe_auto_archive(self, *args: Any, **kwargs: Any): return self._store.maybe_auto_archive(*args, **kwargs)
    def maybe_auto_prune_and_vacuum(self, *args: Any, **kwargs: Any): return self._store.maybe_auto_prune_and_vacuum(*args, **kwargs)
    def sweep_orphaned_sessions(self, **kwargs: Any): return self._store.sweep_orphaned_sessions(**kwargs)
    def adopt_orphaned_gateway_session(self, orphan_id: str, donor_id: str): return self._store.adopt_orphaned_gateway_session(orphan_id, donor_id)
    ...
    def delete_session(self, session_id: str, **kwargs: Any) -> bool:
        ...
        with self._store._connection() as connection, connection.cursor() as cursor:
            ...
            if expected is not None and current != set(expected):
                return False
            return bool(self._delete_session_ids(cursor, [session_id]))
    def request_handoff(self, session_id, platform): return self._store.request_handoff(session_id, platform)
    def claim_handoff(self, session_id): return self._store.claim_handoff(session_id)
    def complete_handoff(self, session_id): return self._store.complete_handoff(session_id)
    def fail_handoff(self, session_id, error, **kwargs): return self._store.fail_handoff(session_id, error, **kwargs)
    def get_handoff_state(self, session_id): return self._store.get_handoff_state(session_id)
    def list_pending_handoffs(self): return self._store.list_pending_handoffs()
    def has_pending_handoffs(self): return self._store.has_pending_handoffs()
    def reclaim_stale_running_handoffs(self, error): return self._store.reclaim_stale_running_handoffs(error)
    def bind_telegram_topic(self, **kwargs): return self._store.bind_telegram_topic(**kwargs)
    def delete_telegram_topic_binding(self, **kwargs): return self._store.delete_telegram_topic_binding(**kwargs)
    def get_telegram_topic_binding(self, **kwargs): return self._store.get_telegram_topic_binding(**kwargs)
    def get_telegram_topic_binding_by_session(self, **kwargs): return self._store.get_telegram_topic_binding_by_session(**kwargs)
    def list_telegram_topic_bindings_for_chat(self, **kwargs): return self._store.list_telegram_topic_bindings_for_chat(**kwargs)
    def list_unlinked_telegram_sessions_for_user(self, **kwargs): return self._store.list_unlinked_telegram_sessions_for_user(**kwargs)
    def is_telegram_session_linked_to_topic(self, **kwargs): return self._store.is_telegram_session_linked_to_topic(**kwargs)
    def is_telegram_topic_mode_enabled(self, **kwargs): return self._store.is_telegram_topic_mode_enabled(**kwargs)
    def enable_telegram_topic_mode(self, **kwargs): return self._store.enable_telegram_topic_mode(**kwargs)
    def disable_telegram_topic_mode(self, **kwargs): return self._store.disable_telegram_topic_mode(**kwargs)
    def apply_telegram_topic_migration(self): return self._store.apply_telegram_topic_migration()
    def relabel_telegram_topic_rows(self, rows): return self._store.relabel_telegram_topic_rows(rows)
    def find_profile_less_telegram_topic_rows(self): return self._store.find_profile_less_telegram_topic_rows()
    def register_backend_heartbeat(self, **kwargs): return self._store.register_backend_heartbeat(**kwargs)
    def list_backend_heartbeats(self): return self._store.list_backend_heartbeats()
    def clear_backend_heartbeat(self, backend_id): return self._store.clear_backend_heartbeat(backend_id)
    def prune_stale_heartbeats(self, **kwargs): return self._store.prune_stale_heartbeats(**kwargs)
    def save_gateway_routing_entry(self, *args, **kwargs): return self._store.save_gateway_routing_entry(*args, **kwargs)
    def load_gateway_routing_entries(self, **kwargs): return self._store.load_gateway_routing_entries(**kwargs)
    def replace_gateway_routing_entries(self, *args, **kwargs): return self._store.replace_gateway_routing_entries(*args, **kwargs)
    def delete_gateway_routing_rows(self, rows): return self._store.delete_gateway_routing_rows(rows)
    def list_gateway_routing_rows(self): return self._store.list_gateway_routing_rows()
    def insert_gateway_routing_rows_if_absent(self, rows): return self._store.insert_gateway_routing_rows_if_absent(rows)
    def gateway_routing_entry_for_session(self, session_id): return self._store.gateway_routing_entry_for_session(session_id)
    def find_latest_gateway_session_for_peer(self, **kwargs): return self._store.find_latest_gateway_session_for_peer(**kwargs)
    def record_gateway_session_peer(self, *args, **kwargs): return self._store.record_gateway_session_peer(*args, **kwargs)
    def session_gateway_runtime(self, session_meta): return self._store.session_gateway_runtime(session_meta)
    def find_orphaned_gateway_sessions(self, **kwargs): return self._store.find_orphaned_gateway_sessions(**kwargs)
    def update_session_runtime_lock(self, *args, **kwargs): return self._store.update_session_runtime_lock(*args, **kwargs)
    def session_count_ge(self, n=1): return self._store.session_count_ge(n)
    def acquire_session_turn_lease(self, *args, **kwargs): return self._store.acquire_session_turn_lease(*args, **kwargs)
    def adopt_session_lineage_from(self, *args, **kwargs): return self._store.adopt_session_lineage_from(*args, **kwargs)
    def key_profiles_for_chat(self, *args, **kwargs): return self._store.key_profiles_for_chat(*args, **kwargs)
    def list_never_active_keyed_sessions(self, **kwargs): return self._store.list_never_active_keyed_sessions(**kwargs)
    def prune_never_active_keyed_sessions(self, **kwargs): return self._store.prune_never_active_keyed_sessions(**kwargs)
    def get_session_rich_row(self, *args, **kwargs): return self._store.get_session_rich_row(*args, **kwargs)

    # PostgreSQL administrative parity is intentionally a direct pass-through;
    # SQLite-only file/WAL operations are not emulated here.
    def export_all(self, source=None): return self._store.export_all(source=source)
    def export_session_for_move(self, session_id): return self._store.export_session_for_move(session_id)
    def import_moved_session(self, payload, *, profile_name): return self._store.import_moved_session(payload, profile_name=profile_name)
    def import_sessions(self, sessions): return self._store.import_sessions(sessions)
    def assert_export_safe(self, session_id, max_messages=None): return self._store.assert_export_safe(session_id, max_messages=max_messages)
    def delete_sessions(self, session_ids, sessions_dir=None): return self._store.delete_sessions(session_ids, sessions_dir=sessions_dir)
    def delete_empty_sessions(self, sessions_dir=None): return self._store.delete_empty_sessions(sessions_dir=sessions_dir)
    def count_empty_sessions(self): return self._store.count_empty_sessions()
    def delete_moved_session(self, session_id): return self._store.delete_moved_session(session_id)
    def rekey_profile_state(self, old_name, new_name): return self._store.rekey_profile_state(old_name, new_name)
    def rekey_legacy_main_sessions(self, ids, profile): return self._store.rekey_legacy_main_sessions(ids, profile)
    def purge_profile_state(self, profile): return self._store.purge_profile_state(profile)
    def relabel_sessions_to_key_profile(self, ids): return self._store.relabel_sessions_to_key_profile(ids)
    def sever_crossed_parents(self, ids): return self._store.sever_crossed_parents(ids)
    def find_crossed_profile_sessions(self, owner): return self._store.find_crossed_profile_sessions(owner)
    def backfill_null_session_profiles(self, profile_name): return self._store.backfill_null_session_profiles(profile_name)
    def fts_rebuild_status(self): return self._store.fts_rebuild_status()
    def fts_cjk_rebuild_status(self): return self._store.fts_cjk_rebuild_status()
    def fts_optimize_available(self): return self._store.fts_optimize_available()
    def optimize_fts(self): return self._store.optimize_fts()
    def optimize_fts_storage(self, **kwargs): return self._store.optimize_fts_storage(**kwargs)
    def rebuild_fts(self): return self._store.rebuild_fts()
    def retry_deferred_fts_recovery(self): return self._store.retry_deferred_fts_recovery()
    def fts_rebuild_step(self): return self._store.fts_rebuild_step()
    def fts_cjk_rebuild_step(self): return self._store.fts_cjk_rebuild_step()
    def vacuum(self): return self._store.vacuum()
    def logical_size_bytes(self): return self._store.logical_size_bytes()
    def retag_kanban_worker_sessions(self, workspaces_root): return self._store.retag_kanban_worker_sessions(workspaces_root)
    def find_foreign_import(self, origin): return self._store.find_foreign_import(origin)
    def find_session_by_origin(self, **kwargs): return self._store.find_session_by_origin(**kwargs)
    def get_session_by_origin(self, **kwargs): return self._store.get_session_by_origin(**kwargs)
