...
def _enrich_skill(conn: sqlite3.Connection, skill_name: str) -> bool:
    ...
    Returns True on success.
    """
    # Gate before reading a SKILL.md. This is duplicated with the transport
    # guard so direct and detached paths cannot read source while disabled.
    if not _enrichment_enabled():
...
def _enrich_pending_skills(conn: sqlite3.Connection, limit: int = 10,
    ...
    """
    # Detached workers exit before selecting candidates or reading files while
